...
local function drawTitle()
    if menus[currentMenu] then
        ...
        --DrawSprite("commonmenu", "interaction_bgd", x, y, menuWidth, titleHeight, 0.0, 179, 0, 255, 255)
...
local function drawButton(text, subText)
    ...
    if multiplier then
        ...
        if subText then
            drawText(subText, menus[currentMenu].x + buttonTextXOffset, y - buttonHeight / 2 - buttonTextYOffset + .009, buttonFont, subTextColor, .34, false, shadow, true)
        end
    end
end
...
function WarMenu.CloseMenu()
    if menus[currentMenu] then
        if menus[currentMenu].aboutToBeClosed then
            ...
            setMenuVisible(currentMenu, false)
			if currentMenu == "PPIS:main" then
			TriggerEvent('nottrue')
			end
